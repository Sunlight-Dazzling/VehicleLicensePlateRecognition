# -*- coding: utf-8 -*-
"""
Created on Sun May 23 11:53:46 2021
...
SAVER_DIR = "E:/CarReconition/train-saver/letters/"
...
if __name__ =='__main__' and sys.argv[1]=='train':
    ...
    for i in range(0+10,NUM_CLASSES+10):
        dir = 'E:/CarReconition/train_images/training-set/letters/%s/' % i           # 这里可以改成你自己的图片目录，i为分类标签
    ...
    val_count = 0
    for i in range(0+10,NUM_CLASSES+10):
        dir = 'E:/CarReconition/train_images/validation-set/%s/' % i           # 这里可以改成你自己的图片目录，i为分类标签
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                val_count += 1
...
if __name__ =='__main__' and sys.argv[1]=='predict':
    ...
    with tf.Session() as sess:
        ...
        for n in range(2,3):
            path = "E:/CarReconition/test_images/%s.bmp" % (n)
            print(path)
            img = Image.open(path)
            width = img.size[0]
            height = img.size[1]
